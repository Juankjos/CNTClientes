...
export default function PaymentHistoryPage() {
  ...
  const [data, setData]       = useState<{ pagos: any[]; pagination: any }>({ pagos: [], pagination: {} });
    ...
    <div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => <div key={i} className="h-20 bg-cnt-surface rounded-xl animate-pulse border border-cnt-border" />)}
        </div>
      ) : data.pagos.length === 0 ? (
          ...
          {data.pagos.map((pago: any) => (
            <Link key={pago.id} href={`/CNTClientes/payments/${pago.id}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-500 text-xs capitalize">{pago.categoria} · {pago.metodo_pago}</p>
                <p className="text-gray-600 text-xs mt-0.5">{pago.referencia}</p>
              </div>

              <div className="text-right shrink-0">
                <p className="text-white font-semibold text-sm">${Number(pago.monto).toFixed(2)}</p>
                <span className={`inline-block mt-1 px-2 py-0.5 rounded border text-[10px] uppercase tracking-wider ${ESTATUS_STYLE[pago.estatus] ?? 'bg-gray-800 text-gray-400'}`}>
                  {pago.estatus}
                </span>
              ...
              <svg className="w-4 h-4 text-gray-600 group-hover:text-gray-400 shrink-0 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {data.pagination?.pages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          ...
          <span className="px-4 py-2 text-gray-500 text-sm">{page} / {data.pagination.pages}</span>
          <button disabled={page >= data.pagination.pages} onClick={() => router.push(`?page=${page + 1}`)}
            className="px-4 py-2 bg-cnt-surface border border-cnt-border text-gray-400 rounded-lg text-sm disabled:opacity-40">
            Siguiente →
          </button>
        </div>
