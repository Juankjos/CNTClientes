...
export async function PATCH(req: NextRequest) {
  ...
  const { nombre, apellidos, telefono, empresa, password_nuevo, password_actual } = body;
  ...
  if (password_nuevo) {
    ...
    await pool.execute(`UPDATE usuarios_clientes SET password = ? WHERE id = ?`, [hash, session.user.id]);
  }

  // Actualizar/crear perfil de cliente
  ...
  if (exists.length) {
    await pool.execute(
      `UPDATE clientes_clientes SET nombre=?, apellidos=?, telefono=?, empresa=? WHERE usuario_id=?`,
      [nombre, apellidos, telefono, empresa, session.user.id]
    );
  } else {
    await pool.execute(
      `INSERT INTO clientes_clientes (usuario_id, nombre, apellidos, telefono, empresa, email)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [session.user.id, nombre, apellidos, telefono, empresa, session.user.email]
    );
  }

  await logAction(session.user.id, 'actualizar_perfil', 'usuarios', 'Perfil actualizado');
