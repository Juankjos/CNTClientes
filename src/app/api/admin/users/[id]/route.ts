...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  // NULL literal va directo en el SET, no como parámetro
  if (body.desbloquear) {
  ...
  const values = [...setValues, Number(params.id)];

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await pool.execute(sql, values as any);

  await logAction(session.user.id, 'editar_usuario', 'usuarios', `ID: ${params.id}`);
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  await logAction(session.user.id, 'desactivar_usuario', 'usuarios', `ID: ${params.id}`);
  return NextResponse.json({ ok: true });
}